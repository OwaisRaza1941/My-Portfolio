...
/// The centered header of the contact section: the "📬 GET IN TOUCH" eyebrow
/// pill, the gradient heading with its accent underline, and a one-sentence
/// invitation.
///
/// Mirrors the about, services, skills and projects headers exactly, so the last
/// section on the page opens with the same gesture as every one before it.
class ContactHeader extends StatelessWidget {
  const ContactHeader({
    super.key,
    this.titleFontSize = AppDimensions.aboutTitleFontSize,
    this.subtitleFontSize = AppDimensions.aboutSubtitleFontSize,
    this.maxWidth = AppDimensions.contactHeaderMaxWidth,
  });
      ...
      children: [
        FadeSlideIn(
          ...
          child: const SectionBadge(
            emoji: AppStrings.contactBadgeEmoji,
            label: AppStrings.contactBadge,
          ),
        ),
